  ...
  for (my $it = 0; $it < @t; $it++) {
    ...
    if ($line =~ /^\s*(R\.br\.|R\.|V\.|Ant\.|Benedictio\.* |Absolutio\.* )(.*)/) {
      ...
      my $l = $2;
        ...
        $line = setclass('rubric', $h) . $l;
    }

    #small omitted title
    if ($line =~ /^\s*\!\!\!(.*)/) {
      $l = $1;
      $line = setclass('annotation', $l);
    }
	...
	#large chapter title
      ...
      my $l = $1;              
      my $suffix = '';
      if ($l =~ s/(\{[^:].*?\})//) {$suffix = setclass('annotation', $1);}
      $line = setclass('title', $l) . " $suffix\n";   
	...
	#red line
      ...
      $l = $1;
      $line = setclass('rubric', $l);
    }
    $line =~ s{/:(.*?):/}{setclass('small-rubric', $1)}e;

    #first letter red 
    if ($line =~ /^\s*r\.\s*(.*)/) {
      $line = $1;
      $line = setclass('small-dropcap', substr($line, 0, 1)) . substr($line, 1);
    }
    ...
    if ($line =~ /^(\s*)v\.\s*(.*)/ || $line =~ /(\{\:.*?\:\}\s*)v\.\s*(.*)/) {
      my $prev = $1;
      $line = $2;
      $line = $prev . setclass('dropcap', substr($line, 0, 1)) . substr($line, 1);
    }
...
sub psalm : ScriptFunc {
  ...
  if ($column == 1) {	
    $psalmnum1++;   
    $pnum = setclass('annotation', " [" . $psalmnum1 . "]");
  } else {
    $psalmnum2++;   
    $pnum = setclass('annotation', " [" . $psalmnum2 . "]");
  }
  my $t = '';
  ...
  if ($num > 150 && $num < 300 && @lines)
  {
    ...
    if ($line =~ /\s*[(]?(.*?)\s+[*]/i)
    {
      $t = setclass('rubric', $1) . settone(1) . $pnum;
    }   
  }
  if (!$t) {$t = setclass('rubric',"$str $num") . settone(1) . $pnum; }
  ...
  if (@lines) {
	...
	foreach $line ( @lines ) {
      ...
      if ($psnum == 94 && $line =~ /^\s*\$ant\s*$/) {
        $formatted_antline ||= setclass('rubric', 'Ant.') . " $antline";
        $t .= "\n$formatted_antline";
        next;
      }
      ...
      if ($line =~ /(.*?)(\(.*?\))(.*)/) {
        ...
        $line = $before . setclass('small-rubric',($this));
      }	else {$rest = $line; $line = '';}
	  ...
	  if ($version !~ /Monastic/i) {
        ...
        $rest =~ s/\x{2021}/setclass('small-rubric', "\x{2021}")/e;
	  }
...
sub canticum : ScriptFunc {
  ...
  if (@w = do_read($fname)) {
    $w[0] =~ s/\!//;  
    $w .= setclass('rubric', shift(@w)) . settone(2) . "\n"; 
    foreach $item (@w) {
      if ($item =~ /^([0-9]+\:)*([0-9]+) (.*)/) {
        my $rest = $3;
        my $num = "$1$2";
        $item = setclass('annotation', $num) . " $rest";   
      }
      $w .= "$item\n";
    }
  return $w;
...
sub martyrologium : ScriptFunc {
  ...
  my $t = setclass('title', "Martyrologium ") . setclass('annotation', "(anticip.)") . "\n_\n";
